...
// ─── API Helper ───────────────────────────────────────────────────────────────
const BASE = import.meta.env.VITE_API_URL || "https://shopflow-production-268d.up.railway.app";
const api = {
  ...
  return (
    <div style={{ width:"100vw", height:"100vh", background:"#0f0f11", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'DM Sans',sans-serif", overflow:"hidden", position:"relative" }}>
      ...
      <div style={{ width:"100%", maxWidth:440, padding:48, background:"#141416", borderRadius:24, border:"1px solid #1e1e22", boxShadow:"0 32px 80px rgba(0,0,0,0.5)", animation:"fadeUp 0.5s ease" }}>
        ...
        <h2 style={{ fontFamily:"'Syne',sans-serif", fontSize:24, fontWeight:800, color:"#f0f0f0", marginBottom:6 }}>Welcome back</h2>
        <p style={{ color:"#6b6b7e", fontSize:14, marginBottom:32 }}>Sign in to continue</p>
        <label style={{ display:"block", fontSize:13, fontWeight:600, color:"#8b8b9a", marginBottom:8 }}>Email</label>
        <input className="login-input" type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@example.com" onKeyDown={e=>e.key==="Enter"&&handleLogin()}
        ...
        <div style={{ position:"relative", marginBottom:24 }}>
          <input className="login-input" type={showPass?"text":"password"} value={password} onChange={e=>setPassword(e.target.value)} placeholder="••••••••" onKeyDown={e=>e.key==="Enter"&&handleLogin()}
        ...
        <button onClick={handleLogin} disabled={loading}
          style={{ width:"100%", padding:13, background:loading?"#7c3d12":"linear-gradient(135deg,#f97316,#ea580c)", border:"none", borderRadius:10, color:"#fff", fontSize:15, fontWeight:700, cursor:loading?"not-allowed":"pointer", marginBottom:28, display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
          {loading?<><Spinner/> Signing in…</>:"Sign In →"}
        </button>
        <div style={{ borderTop:"1px solid #1e1e22", paddingTop:24 }}>
          <p style={{ color:"#6b6b7e", fontSize:12, fontWeight:600, textTransform:"uppercase", letterSpacing:1, marginBottom:12 }}>Demo accounts — click to fill</p>
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            {[
              { label:"👑 Admin",            email:"admin@shopflow.com", password:"admin123" },
              { label:"👤 Aisha (Customer)", email:"aisha@gmail.com",    password:"pass123"  },
              { label:"👤 Raj (Customer)",   email:"raj@gmail.com",      password:"pass123"  },
            ].map(d=>(
              <button key={d.email} className="demo-btn" onClick={()=>{setEmail(d.email);setPassword(d.password);}}
                style={{ background:"#18181c", border:"1px solid #2a2a2e", borderRadius:8, padding:"9px 14px", color:"#d0d0d8", fontSize:13, cursor:"pointer", textAlign:"left", transition:"border-color 0.2s" }}>
                {d.label} — <span style={{ color:"#6b6b7e" }}>{d.email}</span>
              </button>
            ))}
          </div>
        </div>
